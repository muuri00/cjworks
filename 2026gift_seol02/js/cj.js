function cj() {
  ...
  function hasOwl() {
    return !!(jQuery && jQuery.fn && typeof jQuery.fn.owlCarousel === "function");
  }
  ...
  base.module = {
    ...
    control: {
      computeTotalPage: function () {
        ...
        if (m.totalpage !== prev) {
          if (m.page > m.totalpage) m.page = m.totalpage;
          c.arrow();
        }
      },
      ...
      init: function () {
        ...
        $("#catalogue").on("mouseenter focusin mouseleave focusout", function (e) {
          ...
          switch (e.type) {
            case "mouseenter":
            case "focusin":
              $(".arrow").stop(true).animate({ opacity: 1 }, 500);
              break;
            case "mouseleave":
            case "focusout":
              $(".arrow").stop(true).animate({ opacity: 0 }, 500);
              break;
          }
        });
        $(".arrow").on("click", function () {
          var fn = c[$(this).attr("action")];
      ...
      loader: function (url, params) {
        ...
        $.ajax({
          ...
          error: function (xhr) {
            ...
            m.loadingPending = true;
            try {
              console.warn("AJAX error:", url, xhr && xhr.status);
            } catch (e) {}
          }
        });
      },
      ...
      loadComplete: function (data, params) {
        c.computeTotalPage();
        var viewFn = v[params.type];
        if (typeof viewFn === "function") {
          viewFn(data, params);
        }
        try {
          if (params && params.pidx) {
            var code = m.pagecode[Number(params.pidx)] || "page" + params.pidx;
            location.hash = code;
          }
        } catch (e) {}
      ...
      connect: function (list) {
        ...
        var target = parseInt(pagelist[0], 10);
        if (!Number.isFinite(target)) return;
        if (target < 1 || target > m.totalpage) return;
        m.page = target;
        m.dispatchEvent("pageChange", target);
        c.contents(target);
      },
      ...
      direct: function () {
        ...
        if (locurl.indexOf("#") != -1) {
          var hash = locurl
            .substr(locurl.indexOf("#") + 1)
            .replace(" ", "")
            .replace(";", "");
          var mnum = hash.match(/^page(\d+)$/i);
          if (mnum) {
            var n = parseInt(mnum[1], 10);
            if (Number.isFinite(n) && n >= 1 && n <= m.totalpage) {
              c.connect(String(n));
              return;
            }
          }
    ...
    view: {
      ...
      contents: function (data) {
        ...
        if ($old && $old.length) {
          $old.css({ opacity: 0, visibility: "hidden", pointerEvents: "none", zIndex: 0 });
        }

        var $bg = $ghost.find(".bg");
        if ($bg.length) {
          var src = $bg.attr("src") || $bg.attr("data-src") || "";
          if ($bg.attr("data-src") && !$bg.attr("src")) {
            $bg.attr("src", src);
          }
        }
        ...
        Promise.all([bgPromise, waitForImages($primeThumbs, 1200)]).then(function () {
          ready();
        });
      },
      ...
      preview: function (data) {
        ...
        $(".popup")
          .css("opacity", 0)
          .stop()
          .animate({ opacity: 1 }, 400, "easeInOutQuart", function () {
            fitPreviewGrid();
            if (m.loadingPending) {
              v.loading(false);
              m.loadingPending = false;
  ...
  window.detail = function (cate, idx) {
    c.loader(m.path + "detail.html", { type: "detail", cate: Number(cate), idx: Number(idx) });
    return false;
  };

  window.preview = function () {
    c.loader("inc/preview.html", { type: "preview" });
    return false;
  };

  function fitPreviewGrid() {
    var $box = $(".popup .preview");
    ...
    $grid.css({
      ...
      gap: GAP + "px",
      transform: "none",
      width: COLS * ITEM_W + (COLS - 1) * GAP + "px",
      margin: "0 auto"
    });
    var wrapW = $wrap.innerWidth();
    var fullW = COLS * ITEM_W + (COLS - 1) * GAP;
    var left = Math.max(0, (wrapW - fullW) / 2);
